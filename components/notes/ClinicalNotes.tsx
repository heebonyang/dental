  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        />
        <button onClick={handleAdd} className="px-3 py-1 bg-blue-500 text-white text-sm rounded">
          추가
        </button>
      </div>
      <ul className="space-y-2">
        {record.notes.map((note) => (
          <li key={note.id} className="text-sm border border-gray-100 rounded p-2 flex justify-between gap-2">
            <span className="text-gray-700">{note.content}</span>
            <button onClick={() => deleteNote(note.id)} className="text-red-400 hover:text-red-600 shrink-0">✕</button>
          </li>
        ))}
      </ul>
    </div>
