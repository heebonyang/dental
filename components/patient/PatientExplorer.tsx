...
export default function PatientExplorer() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Panel header */}
      <div className="px-3 py-2 bg-gray-900 flex items-center justify-between shrink-0">
        <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">환자 탐색기</span>
        <button onClick={addRecord} className="text-xs text-blue-400 hover:text-blue-300 font-medium">
          + 추가
        </button>
      </div>

      {/* Search */}
      <div className="px-2 py-2 bg-gray-900 border-b border-gray-700 shrink-0">
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="환자 검색..."
          className="w-full text-xs bg-gray-800 border border-gray-700 text-gray-200 placeholder-gray-600 rounded px-2 py-1.5 focus:outline-none focus:border-blue-500"
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-900">
        {filtered.map((rec) => {
          ...
          return (
            <div
              ...
              className={`px-3 py-2.5 cursor-pointer border-b border-gray-800 flex items-center justify-between group transition-colors ${
                isActive
            ...
            >
              <div className="min-w-0">
                <div className={`text-sm font-medium truncate ${isActive ? "text-blue-300" : "text-gray-300"}`}>
        ...
        {filtered.length === 0 && (
          <div className="px-3 py-4 text-xs text-gray-600 text-center">검색 결과 없음</div>
        )}
      </div>
          ...
          {editing ? (
            ...
            <table className="w-full">
              <tbody>
                ...
                <InfoRow label="성별"     value={active.patient.gender === "male" ? "남성" : active.patient.gender === "female" ? "여성" : "기타"} />
